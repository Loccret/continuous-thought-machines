...
from data.custom_datasets import TowerOfHanoiDataset
from tasks.image_classification.plotting import plot_neural_dynamics
from models.utils import reshape_predictions, get_latest_checkpoint
from tasks.hanoi.plotting import make_hanoi_gif
from tasks.hanoi.utils import prepare_model, reshape_attention_weights, reshape_inputs
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train CTM on Tower of Hanoi Task")

    # Model Architecture 
    parser.add_argument('--model_type', type=str, default="ctm", choices=['ctm', 'lstm'], help='The type of model to train.')
    parser.add_argument('--hanoi_disks', type=int, default=3, help='Number of disks for Tower of Hanoi.')
    ...
    parser.add_argument('--log_dir', type=str, default='logs/hanoi', help='Directory to save logs and checkpoints.')
...
def hanoi_loss(predictions, targets):
    """Compute loss for Tower of Hanoi task."""
    # predictions: [batch_size, hanoi_disks, iterations]
    # targets: [batch_size, hanoi_disks]
    # Use the final time step prediction
    final_predictions = predictions[:, :, -1]  # [batch_size, hanoi_disks]
    return torch.nn.functional.mse_loss(final_predictions, targets.float())


def hanoi_accuracy(predictions, targets):
    """Compute accuracy for Tower of Hanoi task."""
    # predictions: [batch_size, hanoi_disks, iterations]
    # targets: [batch_size, hanoi_disks]
    # Use the final time step prediction
    final_predictions = predictions[:, :, -1]  # [batch_size, hanoi_disks]
...
def train_epoch(model, dataloader, optimizer, device, args):
    ...
    for batch_idx, (inputs, targets) in enumerate(pbar):
        ...
        # Reshape predictions: (B, hanoi_disks, Ticks)
        predictions = predictions_raw.reshape(predictions_raw.size(0), args.hanoi_disks, predictions_raw.size(-1))
        
        loss = hanoi_loss(predictions, targets)
        accuracy = hanoi_accuracy(predictions, targets)
...
def main():
    ...
    # Create datasets
    train_dataset = TowerOfHanoiDataset(args.hanoi_disks)
    val_dataset = TowerOfHanoiDataset(args.hanoi_disks)
    ...
    # Create model
    prediction_reshaper = [-1, args.hanoi_disks]  # Problem specific

    # Set output dimensions for the task
    args.out_dims = args.hanoi_disks

    model = prepare_model(prediction_reshaper, args, device)    # Create optimizer
    ...
    if args.grad_clip_quantile > 0:
        optimizer = QuantileClip.as_optimizer(optimizer=optimizer, quantile=args.grad_clip_quantile, history_length=1000)
