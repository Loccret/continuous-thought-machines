...
class ContinuousThoughtMachine(nn.Module):
    ...
    def __init__(self,
        ...
        # --- Plasticity Hyperparameters --- # MODIFIED
        # These hyperparameters control the behaviour of synapse-level and neuron-level plasticity.
        # Plasticity can be toggled on or off via `self.plasticity_enabled`. Learning rates control
        # the strength of Hebbian/Oja updates, decay controls forgetting of fast weights,
        # gamma is the Oja normalization factor, and anti_hebb_lr introduces a small
        # anti-Hebbian term to encourage competition and prevent feature collapse.
        self.plasticity_enabled = True
        self.plastic_lr_syn = 0.1
        self.plastic_decay = 0.9
        self.plastic_gamma = 0.1
        self.anti_hebb_lr = 0.01
        self.plastic_lr_nlm = 0.01
        # --- Input Processing  ---
    ...
    def forward(self, x, track=False):
        B = x.size(0)
        device = x.device  # MODIFIED: keep device for plasticity state
        ...
        # --- Plasticity State Initialisation --- # MODIFIED
        # Initialise fast synaptic weights (H_syn) and fast neuron-level weights for the current forward pass.
        if getattr(self, 'plasticity_enabled', False):
            H_syn = torch.zeros(self.d_model, self.d_model, device=device)
            nlm_fast_weights = torch.zeros(self.d_model, self.memory_length, device=device)
            prev_activated_state_for_plastic = activated_state.clone()
            prev_state_trace_for_plastic = state_trace.clone()
        ...
        for stepi in range(self.iterations):
            ...
            # --- Apply Synapses ---
            # Base synaptic transformation
            base_state = self.synapses(pre_synapse_input)  # theta_syn
            # Add fast synaptic component if plasticity is enabled
            if getattr(self, 'plasticity_enabled', False):
                plastic_pre_synapse = prev_activated_state_for_plastic @ H_syn  # (B, D) # MODIFIED
                state = base_state + plastic_pre_synapse  # MODIFIED
            else:
                state = base_state  # MODIFIED
            ...
            # Compute base neuron-level output
            base_activated_state = self.trace_processor(state_trace)  # N2 # MODIFIED
            if getattr(self, 'plasticity_enabled', False):
                # Add fast neuron-level component from plastic weights
                nlm_addend = (prev_state_trace_for_plastic * nlm_fast_weights.unsqueeze(0)).sum(dim=-1)  # (B, D) # MODIFIED
                activated_state = base_activated_state + nlm_addend  # MODIFIED
            else:
                activated_state = base_activated_state  # MODIFIED
            ...
            # --- Plasticity Updates ---
            if getattr(self, 'plasticity_enabled', False):
                # Perform synaptic and neuron-level plastic updates without gradient tracking
                with torch.no_grad():
                    # Synapse-level Hebbian update with Oja normalization and anti-Hebbian term
                    pre = prev_activated_state_for_plastic  # (B, D) # MODIFIED
                    post = activated_state  # (B, D) # MODIFIED
                    # Outer product averaged across batch
                    hebb = torch.einsum('bi,bj->ij', pre, post) / pre.size(0)  # (D, D) # MODIFIED
                    # Compute per-post normalisation term
                    norm = (post ** 2).mean(dim=0)  # (D,) # MODIFIED
                    # Oja-like update
                    delta_syn = self.plastic_lr_syn * (hebb - self.plastic_gamma * H_syn * norm)  # (D, D) # MODIFIED
                    # Anti-Hebbian term to encourage competition
                    anti_update = -self.anti_hebb_lr * hebb  # MODIFIED
                    # Decay and accumulate plastic updates
                    H_syn = self.plastic_decay * H_syn + delta_syn + anti_update  # MODIFIED
                    # Neuron-level plasticity update (Oja rule per neuron)
                    x = prev_state_trace_for_plastic  # (B, D, M) # MODIFIED
                    y = post  # (B, D) # MODIFIED
                    # Expand dims to align: y[..., None] has shape (B, D, 1)
                    # Update rule: w <- w + lr * mean( y * (x - y * w) )
                    nlm_delta = self.plastic_lr_nlm * ( (y.unsqueeze(-1) * (x - y.unsqueeze(-1) * nlm_fast_weights.unsqueeze(0))).mean(dim=0) )  # (D, M) # MODIFIED
                    nlm_fast_weights = nlm_fast_weights + nlm_delta  # MODIFIED
                # Update previous state trackers
                prev_activated_state_for_plastic = activated_state.clone().detach()  # MODIFIED
                prev_state_trace_for_plastic = state_trace.clone().detach()  # MODIFIED

        # --- Return Values ---
        if track:
            return predictions, certainties, (np.array(synch_out_tracking), np.array(synch_action_tracking)), np.array(pre_activations_tracking), np.array(post_activations_tracking), np.array(attention_tracking)  # MODIFIED
        return predictions, certainties, synchronisation_out  # MODIFIED
